...
const FilterBox = ({ states, hk,isLoading }) => {
  ...
  const AS_OPTIONS = {
    label: "Assigned to",
    options: hk.map((h)=>{return { label: h.fname, value: h.fname}})
  }
  ...
  return (
    ...
    <div className="flex flex-col m-[2rem] rounded-[8px] bg-[#D9D9D9] border-[1px] border-primaryfade shadow-lg">
      ...
      {!isLoading &&  <div className="p-[1rem] flex gap-[1rem] bg-white">
        {filterSelect.map((select, index) => (
          ...
          <div>
            ...
            <MultiSelect
              ...
              value={states.at(-1).state}
              onChange={states.at(-1).setState}
              labelledBy={AS_OPTIONS.label}
            />
          </div>
      </div>}
